...
test.describe('Admin-Navigation (Volunteer-first)', () => {
  test('Einstellungen zeigen Veranstalter, Bestellung und Funktionen-Hinweis', async ({ page }) => {
    ...
    await expect(page.getByRole('link', { name: /Funktionen.*Benachrichtigungen/i })).toBeVisible();
  });
